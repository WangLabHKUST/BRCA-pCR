...
bc_exp_raw = as.data.frame(read.csv("./data/phosphoprotein.csv"))
...
for (K in 1:length(subtypes)){
  ...
  if(fold_calculator == "byMean"){
    foldChanges=log2(rowMeans(dataCon1,na.rm = T)/rowMeans(dataCon2,na.rm = T))
  ...
  write.table(outRst, file=paste("table.5B.DEPPs.",subtype.name,".txt",sep = ""),sep="\t", quote=F,row.names = F,col.names = T)
}
...
updated_Subtype.list = c("ER-HER2+","ER+HER2+","ER-HER2-","all")

# Volcano plots and GSEA analysis
for (i in 1:length(updated_Subtype.list)) {
  subtype.tmp = updated_Subtype.list[i]
  sig_c1 <- as.data.frame(fread(paste("table.5B.DEPPs.",subtype.tmp,".txt",sep = "")))
  ...
  volcano_DEG = ggplot(sig_c1.back, aes(x = log2foldChange, y = -log10(pValues))) +
    ...
    ylim(0, 3.8) +
    labs(title = paste(subtype.tmp,'PhosPro'),
         x = 'Log2 FoldChange', #
         y = '-Log10 (P-value)') +
  ...
  pdf(file = paste0('Figure.5B.DEPPs.volcano_',subtype.tmp, '.pdf'),width = 6,height= 6)
  plot(volcano_DEG)
  dev.off()
